...
// noise drum on MIDI ch10
void noiseOn(uint8_t note, uint8_t vel) {
  uint8_t chip = 0;           // chip 0 for drum
  uint8_t noiseFreq = constrain((int)note - 24, 0, 31);
  psgWrite(chip, 6, noiseFreq);
  // disable tones, enable noise on C
  psgWrite(chip,7,0b11111000);
  psgWrite(chip,8+2, vel>>3);
}
void noiseOff() {
  uint8_t chip = 0;
  enableTones(chip);          // restore tone-only mixer
  psgWrite(chip,8+2,0);
}

// apply pitch bend + vibrato + expression to active voices
void updatePitchMod(uint8_t channel) {
  ...
  for (int v=0; v<3; ++v) {
    if (!voiceActive[chip][v] || voiceChan[chip][v]!=channel)
      continue;
    uint16_t baseP = noteToPeriod(voiceNote[chip][v]);
    float totalSem = pitchBendSemis[channel] + vibSem;
    uint16_t modP = (uint16_t)(baseP / powf(2.0f, totalSem/12.0f) + 0.5f);
    // scale voiceVol by expression
    uint8_t rawVol = voiceVol[chip][v];       // 0–15
    uint8_t expr = expressionVal[channel];    // 0–127
    uint8_t scaledVol = (rawVol * expr + 63) / 127;
    setVoice(chip, v, modP, scaledVol);
  }
}
...
void noteOn(uint8_t channel, uint8_t note, uint8_t vel) {
  ...
  uint8_t chip = midiToChip[channel];
  uint8_t volume = vel >> 3;  // 0–127 → 0–15
  int8_t v;
...
void pitchBend(uint8_t channel, uint8_t lsb, uint8_t msb) {
  int val = (msb<<7)|lsb;
  float norm = ((float)val - 8192)/8192;
...
void handleMidiMsg(uint8_t status, uint8_t d1, uint8_t d2) {
  ...
  else if (cmd==0xB0 && d1==1)   modWheel[ch]      = d2; // CC1
  else if (cmd==0xB0 && d1==11)  expressionVal[ch] = d2; // CC11
...
void parseSerialMidi(uint8_t b) {
  if (b & 0x80) {
    serStatus = b;
    serState  = S_WAIT_D1;
  } else if (serState==S_WAIT_D1) {
    serD1     = b;
    serState  = S_WAIT_D2;
  } else if (serState==S_WAIT_D2) {
...
void loop() {
  ...
  if (now - last >= 5) {
    ...
    for (uint8_t ch=0; ch<9; ch++)
      if (modWheel[ch]>0 || expressionVal[ch]<127)
        updatePitchMod(ch);
  }
